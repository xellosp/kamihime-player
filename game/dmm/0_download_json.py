...
episodes = dict()
souls = os.listdir(os.path.join(data_directory, "soul").decode('utf8'))
eidolons = os.listdir(os.path.join(data_directory, "eidolon").decode('utf8'))
kamihime_Rs = os.listdir(os.path.join(data_directory, "kamihime_R").decode('utf8'))
kamihime_SRs = os.listdir(os.path.join(data_directory, "kamihime_SR").decode('utf8'))
kamihime_SSRs = os.listdir(os.path.join(data_directory, "kamihime_SSR").decode('utf8'))
...
def download_souls_info():
    ...
    url = 'https://r.kamihimeproject.net/v1/a_jobs'
...
def download_kamihime_episodes_info(info):
    ...
    if r.status_code == 200:
        ...
        if info['rare'] == 'SR' or (info['rare'] == 'SSR' and u'神化覚醒' not in info['name']):
...
def download_kamihime_script(info):
    ### Download and save kamihime scene script into raw_scenario folder.###
    character_name = info['name']
    rarity = info['rare']
...
def download_eidolon_episodes_info(info):
    ### Download and save eidolon info to info_eidolon folder.### 
...
def download_eidolon_script(info):
    ### Download and save eidolon scene script into raw_scenario folder.###
    character_name = info['name']
