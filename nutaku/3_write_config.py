...
def get_character_info(entry, scenario, type):
    with open(os.path.join(data_folder, type, entry, scenario)) as file:
        ...
        data = json.load(file)
        if story_type == 'story':
            stage, level = data['scene_data'][-1]['film'].split('-')[0:2]
            if int(stage) == 0:
                if entry == '1':
                    index = 100001
                else:
...
for character in lst:
    # print entry
    with open(os.path.join(kh_info_folder, character)) as file:
        data = json.load(file)
...
for type in os.listdir(data_folder):
    for character in os.listdir(os.path.join(data_folder, type)):
        ...
        if story_type == 'kamihime' and len(scenarios) > 5:
            print character, 'should be 5 or less files in folder'
            logging.error('should be 5 or less files in folder %s' % character)
            sys.exit()
        elif story_type == 'eidolon' and len(scenarios) > 3:
            print character, 'should be 3 or less files in folder'
            logging.error('should be 5 or less files in folder %s' % character)
            sys.exit()

        for scenario in scenarios:
            with open(os.path.join(data_folder, type, character, scenario)) as file:
                data = json.load(file)

            id = int(scenario.split('_')[0])
            label = 'story_nutaku_' + story_type + '_' + "%03d" % id
            ...
            scene_index = scenarios.index(scenario) + 1
